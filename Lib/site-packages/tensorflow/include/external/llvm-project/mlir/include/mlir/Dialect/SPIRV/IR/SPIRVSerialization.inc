...
Serializer::processOp<::mlir::spirv::INTELJointMatrixLoadOp>(::mlir::spirv::INTELJointMatrixLoadOp op) {
  ...
  if (auto attr = op->getAttr("layout")) {
    operands.push_back(prepareConstantInt(op.getLoc(), Builder(op).getI32IntegerAttr(static_cast<uint32_t>(::llvm::cast<::mlir::spirv::MatrixLayoutAttr>(attr).getValue()))));
  }
...
Serializer::processOp<::mlir::spirv::INTELJointMatrixWorkItemLengthOp>(::mlir::spirv::INTELJointMatrixWorkItemLengthOp op) {
  ...
  if (auto attr = op->getAttr("joint_matrix_type")) {
    operands.push_back(static_cast<uint32_t>(getTypeID(llvm::cast<TypeAttr>(attr).getValue())));
  }
